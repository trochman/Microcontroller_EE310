...
;---------------------
 ...
 MOVLW 0x00
 MOVWF TRISD,A
 MOVLW 25 ; keypad reference input temp
 MOVWF 0x21 ; register to store 25 ; keypad reference input temp,A
 ;CALL DECIMAL
 MOVWF 0x21 ; register to store 25 ; keypad reference input temp,A
 MOVLW 45 ; measured sensor input temp
 ;BN NEGM
 MOVWF 0x20 ; register to store 45 ; measured sensor input temp,A
 ;CALL DECIMAL
 SUBWF 0x21 ; register to store 25 ; keypad reference input temp
 BN COOL
 BZ NOTHING
HEAT: MOVLW 0x01
 MOVWF 0x22 ; register to store comparison result,A
 MOVWF PORTD.1 ; port to turn on the heater
 MOVLW 0x00
 MOVWF PORTD.2 ; port to turn on the air conditioning
 SLEEP
NOTHING:MOVLW 0x00
 MOVWF 0x22 ; register to store comparison result,A
 MOVWF PORTD.1 ; port to turn on the heater
